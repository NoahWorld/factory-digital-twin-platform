...
export const validateCanvasTheme = (value: unknown): CanvasTheme => {
...
export const validateNode = (value: unknown): CanvasNode => {
...
/** Legacy single-page documents are v0. Migration preserves every entity ID. */
export function parseCanvasDocument(value: unknown): CanvasDocument {
  const input = requireObject(value, "canvas");
  if (input.schemaVersion !== undefined && input.schemaVersion !== 0 && input.schemaVersion !== 1) {
    invalid("unsupported_schema_version", `Unsupported canvas schema version: ${String(input.schemaVersion)}.`);
  }
  if (!Array.isArray(input.nodes)) invalid("invalid_canvas_document", "canvas.nodes must be an array.");
  const nodes = (input.nodes as unknown[]).map(validateNode);
  if (new Set(nodes.map((node) => node.id)).size !== nodes.length) invalid("duplicate_canvas_node_id", "Canvas node IDs must be unique.");
  if (!Number.isSafeInteger(input.revision) || Number(input.revision) < 0) invalid("invalid_canvas_revision", "Canvas revision must be a non-negative integer.");
  ...
  return {
    ...
    updatedAt: input.updatedAt === null ? null : requireString(input.updatedAt, "canvas.updatedAt", 80),
    nodes,
    dataBindings,
  };
}
