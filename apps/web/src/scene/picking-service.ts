...
export function createPickingService(camera: Camera, canvas: HTMLCanvasElement) {
  ...
  return (clientX: number, clientY: number, records: InstanceRecord[], paths: Map<Object3D, string>): SceneTarget | null => {
    const result = pick(clientX, clientY, records, paths);
    return result && "instanceId" in result ? result : null;
  };
}

export function createSceneObjectPickingService(camera: Camera, canvas: HTMLCanvasElement) {
  ...
  return (clientX: number, clientY: number, records: InstanceRecord[], paths: Map<Object3D, string>, fluids?: { getObjects(): Object3D[]; pickId(object: Object3D): string | null }): SceneObjectTarget | null => {
    ...
    for (const root of fluids?.getObjects() ?? []) {
