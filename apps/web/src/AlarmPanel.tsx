...
export function AlarmPanel({ projectId,versionId,editable,onClose }:{ projectId:string;versionId?:string;editable:boolean;onClose():void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => { if (fetching) return;fetching = true;try { const to = query.before ? new Date(query.to):new Date(),params = new URLSearchParams({ from:new Date(to.getTime()-query.hours*3600000).toISOString(),to:to.toISOString(),state:query.state,limit:"50" });if (query.before) params.set("before",String(query.before));const result = await request<Records>(`${base}/alarms?${params}`,{ signal:controller.signal });if (!controller.signal.aborted) { lastRangeTo.current = to.toISOString();setRecords(result);setRecordsError(null); } } catch (reason) { if (!controller.signal.aborted) setRecordsError(errorMessage(reason)); } finally { fetching = false; } };
