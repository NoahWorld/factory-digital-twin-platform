...
export function SceneModelEditor({ projectId, scene, editable }: { projectId: string; scene: SceneDefinition; editable: boolean }) {
  ...
  return <>
    ...
    {replacementInstanceId && scene.instances.some((instance) => instance.id === replacementInstanceId) ? <ModelReplacementDialog projectId={projectId} scene={scene} instanceId={replacementInstanceId} onClose={() => setReplacementInstanceId(null)} onModelAdded={(model) => setModels((models) => [model, ...models.filter((item) => item.id !== model.id)])} onApply={(replacement) => !!execute({ type: "instance.replace-resource", sceneId: scene.id, instanceId: replacement.instanceId, expectedAssetId: replacement.expectedAssetId, newAssetId: replacement.newAssetId, objectMap: replacement.objectMap,clipMap: replacement.clipMap,clipTimeScales: replacement.clipTimeScales })} /> : null}
  </>;
}
