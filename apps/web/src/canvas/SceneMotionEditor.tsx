...
const propertyNames: Record<string,string> = { position: "位置路径（米）", rotation: "旋转（度）", scale: "缩放", color: "颜色", opacity: "透明度", visible: "显隐", target: "镜头目标路径（米）", clip: "原生片段时间（秒）" };
...
export function SceneMotionEditor({ projectId, scene, snapshots, editable, onApply, onClose }: { projectId: string; scene: SceneDefinition; snapshots: Record<string,ModelSceneSnapshot>; editable: boolean; onApply: (motions: SceneMotion[]) => boolean; onClose: () => void }) {
  const [clipModels,setClipModels] = useState<Record<string,ModelAsset>>({});
  const [clipReload,setClipReload] = useState(0);
  ...
  const selectedInstance = scene.instances.find((instance) => instance.id === (track && track.type !== "camera" ? track.target.instanceId : scene.instances[0]?.id));
  const nativeModel = selectedInstance ? clipModels[selectedInstance.modelAssetId] : undefined;
  const nativeClips = nativeModel?.inspection.clips?.filter((clip) => clip.inDefaultScene) ?? [];
  const activeClip = track?.type === "clip" ? nativeClips.find((clip) => clip.clipId === track.clipId) : undefined;
  useEffect(() => {
    if (!selectedInstance || clipModels[selectedInstance.modelAssetId]) return;
    const controller = new AbortController(), id = selectedInstance.modelAssetId;
    void request<{ modelAsset: ModelAsset }>(`${modelAssetsPath(projectId)}/${encodeURIComponent(id)}`,{ signal: controller.signal }).then(async (result) => {
      if (result.modelAsset.inspection.animationManifestVersion !== 1 && editable) result = await request<{ modelAsset: ModelAsset }>(`${modelAssetsPath(projectId)}/${encodeURIComponent(id)}/inspect`,{ method: "POST",signal: controller.signal });
      if (!controller.signal.aborted) setClipModels((models) => ({ ...models,[id]: result.modelAsset }));
    }).catch((reason) => { if (!controller.signal.aborted) setMessage(`动画清单读取失败：${errorMessage(reason)}`); });
    return () => controller.abort();
  },[projectId,selectedInstance?.modelAssetId,editable,clipModels,clipReload]);
  ...
  const makeClipTrack = (durationMs: number): Extract<SceneMotionTrack,{ type: "clip" }> => ({ id: crypto.randomUUID(),type: "clip",target: { instanceId: selectedInstance!.id,objectId: null },property: "clip",clipId: nativeClips[0].clipId,easing: "linear",keyframes: [{ timeMs: 0,value: nativeClips[0].startTime },{ timeMs: durationMs,value: nativeClips[0].duration }] });
  const setProperty = (property: SceneMotionTrack["property"]) => {
    if (!track || !motion || track.type === "clip") return;
  ...
  return <dialog className="model-replacement-dialog scene-motion-editor" aria-label="场景动画与路径" ref={dialog} onCancel={onClose}>
    ...
    <div className="scene-motion-body"><section className="scene-motion-config"><fieldset disabled={!editable}>
      ...
      <div className="interaction-fields"><button type="button" disabled={!nativeClips.length || motions.length >= 100} onClick={() => { const durationMs = Math.max(1,Math.min(60000,Math.round((nativeClips[0].duration-nativeClips[0].startTime)*1000))),id = crypto.randomUUID(); const next: SceneMotion = { id,name: nativeClips[0].name || "模型原生动画",version: 1,durationMs,repeat: 1,fill: "restore",tracks: [makeClipTrack(durationMs)] }; setMotions([...motions,next]); setSelected(id); setTrackId(""); }}>从原生片段创建动画</button><button type="button" onClick={() => { if (selectedInstance) setClipModels((models) => { const next = { ...models }; delete next[selectedInstance.modelAssetId]; return next; }); setClipReload((value) => value+1); }}>重试动画清单</button><span>{nativeModel ? nativeModel.inspection.animationManifestVersion === 1 ? `当前模型可用片段 ${nativeClips.length} 个` : "旧报告尚无动画清单，需要编辑者补充检查" : "正在读取模型动画…"}</span></div>
      {motion ? <><label>动画名称<input aria-label="动画名称" value={motion.name} onChange={(event) => update({ name: event.target.value })} /></label><div className="interaction-fields"><label>时长（毫秒）<NumberField aria-label="动画时长" min={1} max={60000} value={motion.durationMs} onCommit={(durationMs) => update({ durationMs,tracks: motion.tracks.map((track) => ({ ...track,keyframes: track.keyframes.map((frame) => ({ ...frame,timeMs: Math.round(frame.timeMs * durationMs / motion.durationMs) })) })) as SceneMotionTrack[] })} /></label><label>重复<NumberField aria-label="动画重复次数" min={1} max={100} value={motion.repeat} onCommit={(repeat) => update({ repeat })} /></label></div>
        ...
        <h3>轨道</h3><div className="interaction-fields"><select aria-label="当前轨道" value={track?.id ?? ""} onChange={(event) => setTrackId(event.target.value)}>{motion.tracks.map((track,index) => <option key={track.id} value={track.id}>{index+1} · {track.type === "camera" ? "镜头" : track.type === "clip" ? "原生片段" : "对象"} · {propertyNames[track.property]}</option>)}</select><button type="button" disabled={!scene.instances.length || motion.tracks.length >= 64} onClick={() => { const track = makeTrack("object",motion.durationMs); update({ tracks: [...motion.tracks,track] }); setTrackId(track.id); }}>添加对象轨道</button><button type="button" disabled={motion.tracks.length >= 64} onClick={() => { const track = makeTrack("camera",motion.durationMs); update({ tracks: [...motion.tracks,track] }); setTrackId(track.id); }}>添加镜头轨道</button><button type="button" disabled={!nativeClips.length || motion.tracks.length >= 64} onClick={() => { const clip = makeClipTrack(motion.durationMs); update({ tracks: [...motion.tracks,clip] }); setTrackId(clip.id); }}>添加原生片段轨道</button></div>
        {track ? <>
          {track.type !== "camera" ? <div className="interaction-fields"><select aria-label="轨道实例" value={track.target.instanceId} onChange={(event) => updateTrack(track.type === "clip" ? { ...track,clipId: "",target: { instanceId: event.target.value,objectId: null } } : { ...track,target: { instanceId: event.target.value,objectId: null } })}>{scene.instances.map((instance) => <option key={instance.id} value={instance.id}>{instance.name}</option>)}</select>{track.type === "object" ? <select aria-label="轨道对象" value={track.target.objectId ?? ""} onChange={(event) => updateTrack({ ...track,target: { ...track.target,objectId: event.target.value || null } })}><option value="">整个实例</option>{objects.map((object) => <option key={object.path} value={object.objectId!}>{object.name || "未命名对象"} · {object.path}</option>)}</select> : null}</div> : null}
          {track.type === "clip" ? <><label>原生片段<select aria-label="模型原生片段" value={track.clipId} onChange={(event) => { const clip = nativeClips.find((clip) => clip.clipId === event.target.value); if (clip) updateTrack({ ...track,clipId: clip.clipId,keyframes: track.keyframes.map((frame) => ({ ...frame,value: clip.startTime + (clip.duration-clip.startTime) * frame.timeMs / motion.durationMs })) }); }}><option value="">选择片段</option>{nativeClips.map((clip) => <option key={clip.clipId} value={clip.clipId}>{clip.name || "未命名片段"} · {clip.animationIndex+1} · {clip.duration.toFixed(2)} 秒</option>)}</select></label><p>源时间 {activeClip?.startTime ?? "—"}–{activeClip?.duration ?? "—"} 秒。关键帧值是片段时间，时间线长度控制播放速度。</p></> : null}
          <div className="interaction-fields"><select aria-label="轨道属性" value={track.property} onChange={(event) => setProperty(event.target.value as SceneMotionTrack["property"])}>{(track.type === "camera" ? ["position","target"] : track.type === "clip" ? ["clip"] : ["position","rotation","scale","color","opacity","visible"]).map((property) => <option key={property} value={property}>{propertyNames[property]}</option>)}</select><select aria-label="轨道插值" value={track.easing} onChange={(event) => updateTrack({ ...track,easing: event.target.value as typeof track.easing })}><option value="linear">线性</option><option value="smooth">平滑进出</option></select><button type="button" onClick={() => update({ tracks: motion.tracks.filter((item) => item.id !== track.id) })}>删除轨道</button></div>
          <h3>关键帧</h3><div className="scene-motion-frame scene-motion-frame-heading"><span>毫秒</span>{Array.isArray(track.keyframes[0]?.value) ? <><span>X</span><span>Y</span><span>Z</span></> : <span>目标值</span>}</div>{track.keyframes.map((frame,index) => <div className="scene-motion-frame" key={index}><NumberField aria-label={`关键帧 ${index+1} 时间`} min={0} max={motion.durationMs} value={frame.timeMs} onCommit={(timeMs) => updateTrack({ ...track,keyframes: track.keyframes.map((item,i) => i === index ? { ...item,timeMs } : item) } as SceneMotionTrack)} />
            {Array.isArray(frame.value) ? frame.value.map((number,axis) => <NumberField key={axis} aria-label={`关键帧 ${index+1} ${["X","Y","Z"][axis]}`} step={.1} value={number} onCommit={(number) => { const value = [...frame.value as MotionVector] as MotionVector; value[axis] = number; changeFrame(index,value); }} />) : typeof frame.value === "boolean" ? <select aria-label={`关键帧 ${index+1} 显隐`} value={String(frame.value)} onChange={(event) => changeFrame(index,event.target.value === "true")}><option value="true">显示</option><option value="false">隐藏</option></select> : typeof frame.value === "number" ? <NumberField aria-label={`关键帧 ${index+1} ${track.type === "clip" ? "片段时间" : "透明度"}`} min={0} max={track.type === "clip" ? activeClip?.duration : 1} step={.05} value={frame.value} onCommit={(value) => changeFrame(index,value)} /> : <input aria-label={`关键帧 ${index+1} 颜色`} type="color" value={frame.value} onChange={(event) => changeFrame(index,event.target.value)} />}
