...
export type BindingCell = { state: "live" | "loading" | "empty" | "null" | "type-error" | "transform-error" | "query-error" | "stale" | "offline"; text: string; value?: RuntimeMetricValue; unit?: string };
export const bindingStateLabels: Record<BindingCell["state"], string> = {
  live: "实时", loading: "读取中", empty: "指标缺失", null: "空值", "type-error": "类型错误", "transform-error": "转换错误", "query-error":"查询错误", stale: "数据陈旧", offline: "设备失联",
};
...
export function readBindingCell(connection: RuntimeAssetConnection | undefined, metric: ComponentBinding["metrics"][number]): BindingCell {
  ...
  if (connection.status === "offline") {
    const state = connection.errorCode === "data_source_stale" ? "stale"
      ...
      : connection.errorCode?.startsWith("sqlite_query_") ? "query-error"
