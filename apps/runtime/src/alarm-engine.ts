...
export class AlarmEngine {
  ...
  query(projectId:string,scopeId:string,input:AlarmQuery) {
    const assetFilter = input.assetIds?.length ? JSON.stringify(input.assetIds):null;
    const active = this.db.statement("SELECT e.id,e.rule_id AS ruleId,e.asset_id AS assetId,e.name,e.severity,e.message,e.occurred_at AS occurredAt,e.ended_at AS endedAt,e.end_reason AS endReason,s.confirmation FROM alarm_episodes e JOIN alarm_states s ON s.active_episode_id=e.id WHERE e.project_id=? AND e.scope_id=? AND (? IS NULL OR e.asset_id IN (SELECT value FROM json_each(?))) AND e.ended_at IS NULL ORDER BY e.occurred_at DESC LIMIT 201").all(projectId,scopeId,assetFilter,assetFilter) as AlarmEpisode[];
    const rows = this.db.statement("SELECT v.id,v.episode_id AS episodeId,e.rule_id AS ruleId,e.asset_id AS assetId,e.severity,e.message,v.kind,v.observed_at AS observedAt,CASE WHEN ?=1 THEN '[]' ELSE v.values_json END AS valuesJson FROM alarm_events v JOIN alarm_episodes e ON e.id=v.episode_id WHERE v.project_id=? AND v.scope_id=? AND (? IS NULL OR e.asset_id IN (SELECT value FROM json_each(?))) AND v.observed_at>=? AND v.observed_at<=? AND (? IS NULL OR v.id<?) AND (?='all' OR (?='active' AND e.ended_at IS NULL) OR (?='ended' AND e.ended_at IS NOT NULL)) ORDER BY v.id DESC LIMIT ?").all(input.summary ? 1:0,projectId,scopeId,assetFilter,assetFilter,input.from,input.to,input.before,input.before,input.state,input.state,input.state,input.limit+1) as Array<Omit<AlarmRecord,"values"|"valuesOmitted">&{ valuesJson:string }>;
    const records = rows.slice(0,input.limit).map(({ valuesJson,...row }) => ({ ...row,values:JSON.parse(valuesJson),...(input.summary ? { valuesOmitted:true }:{}) })) as AlarmRecord[];
    const ruleStates = this.db.statement("SELECT rule_id AS ruleId,json_extract(rule_json,'$.assetId') AS assetId,confirmation,active_episode_id AS activeEpisodeId FROM alarm_states WHERE project_id=? AND scope_id=? AND (? IS NULL OR json_extract(rule_json,'$.assetId') IN (SELECT value FROM json_each(?))) ORDER BY rule_id LIMIT 200").all(projectId,scopeId,assetFilter,assetFilter) as AlarmRuleStatus[];
    ...
    return { configured:this.plans.has(this.scope(projectId,scopeId)),projectId,scopeId,retentionDays:30,maxEvents:200000,ruleStates,active:active.slice(0,200),activeTruncated:active.length > 200,records,nextCursor:rows.length > input.limit ? records.at(-1)!.id:null };
  }
}
