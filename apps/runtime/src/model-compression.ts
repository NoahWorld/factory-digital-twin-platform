import {MODEL_COMPRESSION_ALGORITHM,validateCompressionProvenance,type ModelCompressionProvenance} from "../../../shared/model-compression";
...
export async function compressModelBytes(bytes:Uint8Array):Promise<{bytes:Uint8Array;report:ModelCompressionReport}> {
  ...
  if(json.asset.extras?.newpowerCompression!==undefined) fail("model_compression_already_processed");
  ...
  const outputJson={...json,asset:{...json.asset,extras:{...json.asset.extras,newpowerCompression:provenance}},buffers:[{byteLength:outputOffset},...json.buffers.map(({uri,...buffer}:any)=>({...buffer,extensions:{...buffer.extensions,EXT_meshopt_compression:{fallback:true}}}))],bufferViews:views,extensionsUsed:[...new Set([...(json.extensionsUsed ?? []),"EXT_meshopt_compression"])],extensionsRequired:[...new Set([...(json.extensionsRequired ?? []),"EXT_meshopt_compression"])]};
...
export async function verifyModelCompression(source:Uint8Array,output:Uint8Array):Promise<void> {
  const summary=inspectMeshoptBytes(output);const before=unpack(source),after=unpack(output),provenance=validateCompressionProvenance(after.json.asset?.extras?.newpowerCompression);
