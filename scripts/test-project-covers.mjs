...
const temporary = await mkdtemp(join(tmpdir(), "kingdom-project-covers-"));
try {
  const outfile = join(temporary, "test.mjs");
  await build({
    entryPoints: [fileURLToPath(new URL("../apps/web/tests/project-covers.test.mjs", import.meta.url))],
    outfile,
    bundle: true,
    platform: "node",
    format: "esm",
