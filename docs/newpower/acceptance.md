...
## M6b 历史与告警看板组件检查点（2026-09-13）

起点 `315b714`。组件绑定v2支持折线/面积历史与服务端告警，继续通过共享校验、项目CAS、复制/历史与冻结快照保存。历史曲线使用有界时间分桶，支持均值/最小/最大/末次值；坏数据、混合配置或单位不混成有效曲线。0004仅为独立遥测库增加查询索引，沿同一备份迁移入口；不改Worker配置库。

实际UI从静态组件设置历史窗口、聚合、适用资产和告警范围，保存、重开。告警行选择设备后，指标卡、模型选中与两条历史曲线同用assetId；两条相同曲线共用一个查询周期，切换设备后读到另一台设备的历史。节点静态props始终不变，复制保留v2查询配置，发布版按固定scope运行，离开页面停止查询。新快照声明telemetry-bindings-v1，旧运行器明确拒绝不支持的能力。

第一次副本检查失败是测试在API修改后继续使用旧编辑Provider：实际库revision4、两条430宽曲线均存在，页面仍revision3。测试重新打开加载新修订后完整链通过。另一次API测试把配置变更前后样本放进默认三分钟桶，按约定得到mixed空点；已改为等待新配置的独立秒桶，未放宽混合判定。实际双曲线、告警及固定发布截图已查看。

共享查询同页64种、4并发，超时取消、最后释放、旧响应忽略、容量恢复和终态权限拒绝有检查。只读复核修复了同listener重复订阅提前释放，以及矛盾告警响应会显示无活动告警的问题。客户端同时校验project/scope/资产与活动事件/规则状态一致性；告警列表取summary，SQL不读采样值正文，普通API保留完整值。

- `m6b-widgets-ui-reopened.log`：真实配置/保存/重开/复制、双查询共享、设备联动、故障、发布及离开释放通过。
- `m6b-widget-api-fixed.log`：时间桶、输入契约、资产/版本/权限筛选、摘要与完整值、共享查询6项通过。
- `m6b-widget-regression.log`：完整158项中156通过、1示例跳过；唯一失败为旧柱状图校验文字变化。
- 旧提示已恢复；`m6b-widget-legacy-message.log`：原绑定零值/空值/类型、历史桶和查询生命周期8项复验通过。
...
### M7 生成精确压缩版本 checkpoint（2026-09-19，验收完成）

本次按Owner新请求，只完成暂停位置的下一个checkpoint，然后推送newpower；不启动持续总目标或继续后续M7/M8。

模型版本界面可从现有资源生成Meshopt不可变版本、预览原生动画、保留对象/clip标识、应用替换、保存、撤销重做和导出。编码直接使用官方Meshopt1.1.1处理原始bufferView，索引INDICES保持原顺序，U8视图不压缩，节点/动画/材质及其他JSON字段保留，不进行量化/重排/减面。百万面生成资源24,095,688字节压缩至14,463,816字节（含来源记录，约减少40%），重复输出SHA一致；该样例结果不代表所有模型的压缩率。

专项sep19-optimization-final.log共6项通过，覆盖百万面可重现输出、多buffer/U8、重叠view提前限额、骨骼/形态/动画数据、伪造来源统计拒绝、提交后读回失败仍保留文件、权限撤销、并发拒绝及取消释放、实际界面替换/撤销。UI原测试错将实例按钮列表作为select，已依据实际控件修正。压缩来源写入GLB，包导入核对包含的原版本及每视图实际解码字节；普通上传禁止声称可信派生。

完整回归启动时前端仅监听IPv6，测试使用IPv4导致连接拒绝，已停止该次运行，明确指定127.0.0.1和8791代理并验证页面/认证/mock连通后重跑。任务本地测试启动器增加同地址服务预检。最终结果待追加，不将环境失败计为产品通过。

最终结果：sep19-full-regression-fixed.log完整188项通过、1示例跳过，1项普通上传停机行为回退已修复（仅派生压缩任务在对象写入后继续执行取消检查，普通已接受上传按旧约定收尾）。sep19-verified.log最终15项全通过，覆盖上述停机问题、权限/取消/提交后读回、重叠视图预算、纹理/透明材质/骨骼形态/动画、实际新片段预览完成、替换保存/撤销重做及原服务停止后的独立包运行。check/build、冻结锁安装与Worker smoke12通过。此次Chrome为153，M5/16GiB；未重新声称长期稳定性完成。

本checkpoint完整，不代表M7或总目标完成。按本次Owner授权，提交后仅推送origin/newpower并停止；保留后续LOD/实例优化/GIS/30分钟稳定性及M8待恢复。
