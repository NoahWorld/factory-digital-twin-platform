...
日期：2026-09-09。关联评估：[厂房场景架构评估与技术选型](./厂房场景架构评估与技术选型.md)。本轮仅调整前端；后端、数据库与采集服务保持原状。
...
1. **统一资产状态与原地 2D 面板**：提取页面中的资产订阅与选择状态；2D 面板和 3D 外观消费同一资产快照。先确定跨项目资产授权、稳定编号、快照时间与陈旧语义；当前不会伪造 SSE 服务或实时数据。
