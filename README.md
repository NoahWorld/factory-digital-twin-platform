...
- R2 目标绑定：`PROJECT_FILES` → `factory-digital-twin-project-files`（统一保存模型与图片；对象键按资源类型和项目隔离）

## 已实现的第一期基础能力

- 用户身份：首个平台管理员初始化、邮箱密码登录、服务端会话 Cookie 与退出登录。
- 权限边界：平台管理员、交付负责人、只读用户三个全局角色；项目内另有负责人、可编辑、只读成员关系。
- 项目：受权限保护的项目列表与新建草稿项目；创建人自动成为项目负责人。项目负责人和平台管理员可删除项目，拥有编辑权限的成员可修改名称；删除会同步清理画布、模型/图片元数据、对象存储文件、资产、数据源和成员关系，对象存储清理异常会明确返回警告。
- 2D 画布：固定 1920 × 1080 交付画布，支持折线图、柱状图、指标卡、环形进度、进度排行、状态矩阵、矩形、圆形、大屏标题、背景点缀、时间日期、标题、小卡片背景、小图标背景，以及纯文本、文字超链接、图片、轮播图、按钮、Switch、多选框、单选框和下拉菜单从组件面板拖入。纯文本支持关闭、水平和垂直滚动；图片和轮播图使用独立项目资源，链接、按钮和表单控件只在预览态响应。组件移动时自动吸附并显示辅助线，选中后可从四角自适应缩放并配置内容、颜色和静态数据。工具栏可一键应用深色、浅色或用户自选色主题，画布与所有非 3D 组件会联动更新，3D 模型的背景、灯光和材质配置保持独立；主题修改不会自动保存，确认后仍需显式保存画布。
...
- 数据源配置：画布工具栏可打开项目级数据源面板，创建和修改 REST 轮询或 WebSocket 连接元数据。REST 配置保存轮询周期、请求超时与可选的源时间戳 JSON 路径，WebSocket 配置保存心跳与最大重连间隔；明文账号、密码、Token 和 API Key 不进入 URL 或前端配置，只允许引用服务端凭据。P1 新增了显式开关和主机白名单保护的本地 REST 即时采集端点；WebSocket 执行、凭据解析和生产级调度仍未实现。
...
账号未启用 R2 时，云验证配置不声明 `PROJECT_FILES` 绑定：页面、身份、项目、模板与不依赖文件的 2D 画布仍可部署，但 `/health` 明确返回 `degraded`，模型与图片上传/读取分别返回 `503 model_storage_not_configured` 或 `503 image_storage_not_configured`。不得把文件改存 D1 或用假成功绕过这一限制。
