...
- 数据源配置：画布工具栏可打开项目级数据源面板，创建和修改 REST 轮询或 WebSocket 连接元数据。REST 配置保存轮询周期与请求超时，WebSocket 配置保存心跳与最大重连间隔；明文账号、密码、Token 和 API Key 不进入 URL 或前端配置，只允许引用服务端凭据。P1 新增了显式开关和主机白名单保护的本地 REST 即时采集端点；WebSocket 执行、凭据解析和生产级调度仍未实现。
- 资产指标映射：在独立 3D 编辑器中为已绑定资产配置项目数据源、唯一指标键、JSON 字段路径、值类型、单位与数据过期时间。映射独立按行保存，不进入画布 JSON。预览态会按映射读取 REST JSON，严格校验字段与类型，并通过同一 `assetId` 驱动模型状态颜色和点击后的 2D 指标详情；采集失败会保留最后成功值用于诊断，同时明确标记设备失联和重连次数。
