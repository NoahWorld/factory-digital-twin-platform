...
- 2D 画布：固定 1920 × 1080 交付画布，支持折线图、柱状图、指标卡、环形进度、进度排行、状态矩阵、矩形、圆形，以及大屏标题、背景点缀、时间日期、标题、小卡片背景和小图标背景从组件面板拖入；组件移动时自动吸附并显示辅助线，选中后可从四角自适应缩放并配置内容、颜色和静态数据，保存后可进入只读预览。
- 大屏模板：工作台顶部“模板”进入 `#/templates` 模板中心，也可从画布工具栏就近打开；内置装备修理、装备保障、生产运营、能耗安全 4 套可编辑模板。选择目标项目后会进入画布并替换当前本地布局，但不会自动保存；所有预设指标都标记为“示例数据”，装备保障模板会复用当前画布的首个 3D 模型资源。
...
本地数据库迁移包括 `0001_initial.sql`（项目、资产与数据配置）、`0002_access_control.sql`（用户、角色、会话与项目成员）、`0003_canvas_foundation.sql`（画布元数据与组件节点）、`0004_canvas_shape_nodes.sql`（基础图形）、`0005_canvas_decoration_nodes.sql`（大屏点缀组件）、`0006_model_assets_and_3d_node.sql`（模型资源与 3D 组件）和 `0007_dashboard_components.sql`（指标卡、环形进度、进度排行与状态矩阵）。云端 D1 尚未执行这些业务迁移。远程部署前先创建 R2 桶：
