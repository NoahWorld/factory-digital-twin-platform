...
- 2D 画布：固定 1920 × 1080 交付画布，支持折线图、柱状图从组件面板拖入，组件移动时自动吸附并显示辅助线；选中组件后可从四角缩放，并在属性面板配置标题、单位、颜色和静态分类数据，保存后可进入只读预览。
...
本地数据库迁移包括 `0001_initial.sql`（项目、资产与数据配置）、`0002_access_control.sql`（用户、角色、会话与项目成员）和 `0003_canvas_foundation.sql`（画布元数据与组件节点）。云端 D1 尚未执行这些业务迁移，部署前必须显式运行：
