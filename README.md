...
- 3D 组件：从组件面板拖入 3D 模型视窗，可导入 GLB 或自包含 GLTF，服务端检查格式、版本、文件体积、节点/网格/材质/纹理/动画数量与重复节点名；模型资源写入 R2，画布只保存资源 ID。编辑态可配置节点位置、旋转、缩放、材质颜色、透明度与显隐，以及背景、灯光、网格、视角和自动旋转；预览态支持鼠标旋转与缩放视角。
